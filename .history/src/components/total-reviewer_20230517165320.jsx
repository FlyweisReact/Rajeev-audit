...
const TotalReviewer = ()=>{
    ...
    const url = "https://63munf68y8.execute-api.ap-south-1.amazonaws.com/dev/api/v1/reviewer/reviewers";
    ...
    function MyVerticallyCenteredModal(props) {
      ...
      const searchData = !query ? reviewer :
        reviewer?.filter((item)=>{
            return item?.reviewerId?.includes(query) ||
                item?.reviewerName?.toLowerCase()?.includes(query?.toLowerCase());
        })
                    ...
                    <div className="totalsitecont2">
                        <div className="totalinspeccont">
                            ...
                            <div className="totalinspeccontr">
                                <button className="btn1" onClick={() => setModalShow(true)}>Add Reviewer</button>
                            ...
                            <tbody>
                                {
                                    searchData?.map((ele,i)=>(
                                        <tr>
                                            <td>{ele?.reviewerId}</td>
                                            <td>{ele?.reviewerName}</td>
                                            <td>{!ele?.reviewStatus?"on-going":"completed"}</td>
                                            <td>{ele?.reviewRemarks}</td>
                                        </tr>
                                    ))
                                }
                            </tbody>
